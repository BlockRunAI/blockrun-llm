...
def test_public_service_account_error_contract(case, status, monkeypatch):
    ...
    def handler(request):
        ...
        return httpx.Response(
            status,
            json={"error": {"code": "fixture_limit", "message": KEY}},
    ...
    try:
        ...
        assert failure.value.response["code"] == "fixture_limit"
        assert KEY not in str(failure.value.response)
        assert len(seen) == 1
    finally:
        client.close()
...
@pytest.mark.parametrize("status,state", [(402, "failed"), (200, "failed"), (200, "cancelled")])
def test_accepted_media_failure_never_resubmits_or_signs(cls, status, state, monkeypatch):
    monkeypatch.setenv("BLOCKRUN_API_KEY", KEY)
    monkeypatch.delenv("BLOCKRUN_API_BASE_URL", raising=False)
    ...
    def handler(request):
        ...
        if len(calls) == 1:
            return httpx.Response(
                202, json={"status": "queued", "poll_url": "/api/v1/jobs/fixture"}
            )
        assert request.url.path == "/v1/jobs/fixture"
        return httpx.Response(
            status,
            json={"status": state, "error": {"message": "fixture"}},
            headers={"payment-required": "must-not-sign"},
        )
    ...
    try:
        with pytest.raises(APIError) as failure:
            client.generate("test fixture")
        assert failure.value.status_code == (402 if status == 402 else 502)
